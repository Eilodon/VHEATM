---
name: vheatm
description: Use VHEATM as an executable audit control plane when the user asks to audit, review, or evaluate a repository against security/architecture gates, or wants an evidence-gated, machine-validated audit report instead of freeform review.
---
...
1. Collect an audit context that conforms to `schemas/audit-context.schema.json`. Run `vheatm-init --root .` (or the `vheatm_evaluate` MCP tool with a starter context) if none exists yet.
...
When this plugin's MCP server is connected, the equivalent tools are
`vheatm_validate`, `vheatm_evaluate`, `vheatm_route`, and
`vheatm_validate_report` — prefer them over shelling out to the CLI.
...
The registry and each instruction file are digest-bound. A content change requires a corresponding contract and registry update. Digest mismatch is a validation failure, not a warning. Run `vheatm-doctor --fix` to recompute and repair a stale digest after an intentional edit.
